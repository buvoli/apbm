...
module imexpbdf_mod
    ...
    subroutine imexpbdf(L, N, tspan, y_in, Nt, settings, y_out, times)
        ...
        do i = 1, Nt
            ...
            RHS = matmul(Y, PBDFW(1:q, :)) + matmul(YN, r * EDW);
            ! compute qth output    
            Y(:, q) = RHS(:, q) / (1 - r * PBDFW(q+1,q) * L)
            ! explicitly compute remaining outputs
            do j = 1, q-1
                Y(:, j) = RHS(:, j) + r * L * Y(:,q) * PBDFW(q+1, j)
            enddo
            ! evaluate RHS
            do j = 1, q
                call N(t0 + h * settings%z(j), Y(:,j), YN(:,j))
            enddo            
            t0 = tspan(1) + h * i;
        enddo
